...
def read_ffd_xyz(filename: str) -> np.ndarray:
    """
    Read FFD control points from a .xyz file.
    ...
    try:
        with open(filename, 'r') as f:
            ...
            try:
                ...
                logger.info(f"File indicates {num_points} control points")
            except ValueError:
                logger.warning("First line doesn't contain valid number of points")
                num_points = None
        ...
        if num_points is not None and len(points) != num_points:
            logger.warning(f"File indicates {num_points} points but {len(points)} were read")
