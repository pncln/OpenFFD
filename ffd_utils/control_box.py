...
def create_ffd_box(mesh_points: np.ndarray,
                   control_dim: tuple = (4, 4, 4),
                   margin: float = 0.0,
                   custom_dims: Optional[List[Tuple[float, float]]] = None) -> Tuple[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
    """
...
def extract_patch_points(mesh_data: Any, patch_name: str) -> np.ndarray:
    ...
    This function is just a wrapper to avoid circular imports.
    See mesh_readers.general_reader.extract_patch_points for implementation.
    ...
    # Import the function from general_reader to avoid circular imports
    from mesh_readers.general_reader import extract_patch_points as _extract_patch_points
    try:
