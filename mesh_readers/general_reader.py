...
def extract_patch_points(mesh_data, patch_name: str) -> np.ndarray:
    """
    ...
    # 3) Try cell_data for direct labels
        ...
        for key in mesh.cell_data:
            ...
            if found_in_cell_data:
                logger.debug(f"Found patch in cell_data[{key}]: {patch_name}")
                break
    ...
    if not found:
        ...
        if hasattr(mesh, 'cell_sets'):
            available += list(mesh.cell_sets.keys())
        if hasattr(mesh, 'field_data'):
            available += list(mesh.field_data.keys())
        if hasattr(mesh, 'cell_data'):
            # Add cell data keys with potential values
            for key in mesh.cell_data:
                for data_array in mesh.cell_data[key]:
                    if len(data_array) > 0:
                        if isinstance(data_array[0], (str, bytes)):
                            unique_values = np.unique(data_array)
                            available += [f"{key}:{val}" for val in unique_values[:10]]  # Limit to first 10
                        else:
                            available += [f"{key}:numeric_values"]
        
        error_msg = f"Patch '{patch_name}' not found. Available sets/groups: {available}"
        logger.error(error_msg)
        raise ValueError(error_msg)
    
    if not node_ids:
        error_msg = f"Patch '{patch_name}' contains no nodes"
        logger.error(error_msg)
        raise ValueError(error_msg)
        
    indices = np.fromiter(node_ids, dtype=int)
